...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    if command -v psql &> /dev/null; then
        ...
        if node ticru-cli.js init-db; then
            echo -e "${GREEN}✓ Database initialized${NC}"
        else
            echo -e "${YELLOW}⚠ Database initialization failed${NC}"
            echo "You can run 'node ticru-cli.js init-db' later"
        fi
    else
...
chmod +x ticru-cli.js
chmod +x build-system.js
...
echo "    • node ticru-cli.js run       - Start both frontend & backend"
echo "    • npm run cli run             - Alternative command"
echo ""
echo "  ${BLUE}Development:${NC}"
echo "    • npm run dev                 - Start frontend dev server"
echo "    • npm run api:dev             - Start API server"
echo "    • node ticru-cli.js serve     - Start API server (alternative)"
echo ""
echo "  ${BLUE}Building:${NC}"
echo "    • npm run build               - Build for production"
echo "    • node build-system.js --all  - Complete build pipeline"
echo ""
...
echo ""
echo "  ${BLUE}Tools:${NC}"
echo "    • node ticru-cli.js --help    - CLI commands"
echo "    • node ticru-cli.js status    - Check app status"
echo ""
